...
func TestJWKSValidatorCachesFetchedKey(t *testing.T) {
	generated, err := jwtgen.Generate(jwtgen.Config{
		Issuer: "api-gateway", Audience: "go-service-template", TokenUse: "access",
		Scope: "greeting.read", KeyID: "key-1", TTL: time.Hour,
	})
	if err != nil {
	...
	validator := newJWKSValidator("https://jwks.example.test/keys", "api-gateway", "go-service-template", client)
